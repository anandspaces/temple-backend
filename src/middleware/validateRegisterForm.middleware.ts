...
export function validateRegisterForm(req: Request, res: Response, next: NextFunction) {
  ...
  const medicalConditions = medicalConditionsRaw
    ? medicalConditionsRaw.split(",").map((s) => s.trim()).filter(Boolean)
    : [];
  const formData = {
    ...body,
    emergencyContact,
  ...
  (req as Request & { validatedBody: RegisterBody }).validatedBody = result.data;
  next();
}
