...
const UserModel = User as { findOne: (q: { _id: unknown }) => ReturnType<typeof User.findOne>; findOneAndUpdate: (q: { _id: unknown }, u: UpdateUserBody, o: { new: true }) => ReturnType<typeof User.findOneAndUpdate>; findOneAndDelete: (q: { _id: unknown }) => ReturnType<typeof User.findOneAndDelete> };
...
export async function updateMe(req: ReqWithUpdateBody, res: Response) {
  ...
  const updated = await UserModel.findOneAndUpdate({ _id: userId }, updates, { new: true }).lean();
  if (!updated) {
