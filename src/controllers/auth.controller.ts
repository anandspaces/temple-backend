...
type ReqWithRegister = Request & {
  ...
  files?: { aadhaarFile?: Express.Multer.File[]; profileAvatar?: Express.Multer.File[] };
};

async function getFileUrls(files: ReqWithRegister["files"]): Promise<{ aadhaarIdFileUrl: string; profileAvatarUrl: string }> {
...
export async function verifyOtp(req: ReqWithValidated, res: Response) {
  ...
  if (!valid) {
    logger.warn({ phone: maskPhone(phoneNumber!) }, "Verify OTP failed: invalid or expired");
  ...
  logger.info({ phone: maskPhone(phoneNumber) }, "Phone verified for registration");
...
export async function login(req: ReqWithValidated, res: Response) {
  ...
  const valid = verify(phoneNumber!, otp!);
  if (!valid) {
    logger.warn({ phone: maskPhone(phoneNumber!) }, "Login failed: invalid or expired OTP");
    return res.status(401).json(apiError("Invalid or expired OTP"));
  }
  const user = await User.findOne().where("phoneNumber", phoneNumber!).select("_id fullName phoneNumber").lean();
  if (!user) {
    logger.warn({ phone: maskPhone(phoneNumber!) }, "Login failed: user not found");
    return res.status(404).json(apiError("User not found. Please register first."));
  }
  try {
    ...
    logger.info({ userId: user._id, phone: maskPhone(phoneNumber!) }, "Login success");
    return res.status(200).json(
      apiSuccess({
        accessToken,
        expiresIn,
...
export async function register(req: ReqWithRegister, res: Response) {
  ...
  if (existing?.isPhoneVerified) {
    ...
    await existing.save();
    logger.info({ userId: existing._id, phone: maskPhone(body.phoneNumber) }, "Register success (update)");
    return res.status(200).json(
      apiSuccess({
        uid: existing._id,
        isVerified: true,
  ...
  if (existing && !existing.isPhoneVerified) {
    ...
    logger.info({ phone: maskPhone(body.phoneNumber) }, "Register: OTP sent, awaiting verification");
    return res.status(200).json(
      apiSuccess({
        uid: existing._id,
        isVerified: false,
  ...
  if (!user) {
    logger.error({ phone: maskPhone(body.phoneNumber) }, "Register failed: user create returned null");
    return res.status(500).json(apiError("Failed to create user"));
  }

  await generateAndStore(body.phoneNumber);
  logger.info({ userId: user._id, phone: maskPhone(body.phoneNumber) }, "Register: OTP sent, awaiting verification");
  return res.status(200).json(
    apiSuccess({
      uid: user._id,
      isVerified: false,
